...
import { Briefcase, MapPin, Building2, ExternalLink, Loader2, Clock, AlertCircle } from 'lucide-react';
...
export default function JobsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // Get all jobs that are not expired (created_at > 30 days ago)
      ...
      if (!error && jobsData) {
        // Sort to prioritize jobs from same course
        const sortedJobs = [...jobsData].sort((a, b) => {
          if (!data) return 0; // If no user profile, don't sort
          const aCourse = a.profiles?.course_name;
          const bCourse = b.profiles?.course_name;
          const userCourse = data.course_name;
  ...
  const handlePostJob = async (e) => {
    ...
    if (!error) {
      ...
      // Refresh the job list after posting
      const thirtyDaysAgo = new Date();
      thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        {canPostJob && (
          <button 
            ...
            className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:opacity-90 hover:-translate-y-0.5 transition-all duration-300 shadow-lg shadow-blue-500/30"
          >
              ...
              <div key={job.id} className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-gray-200/50 dark:border-slate-700/50 rounded-2xl p-6 hover:border-blue-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-blue-900/5 hover:-translate-y-1 flex flex-col md:flex-row gap-6 group relative overflow-hidden">
                ...
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2 pr-16">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-600 group-hover:to-indigo-600 transition-all">{job.title}</h3>
                    <div className="flex gap-2">
                      <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400">Full-time</span>
                  ...
                  {job.profiles?.course_name && (
                    <div className="text-sm font-medium text-indigo-600 dark:text-indigo-400 mb-4 bg-indigo-50 dark:bg-indigo-900/20 inline-block px-2 py-0.5 rounded-md">
                ...
                <div className="flex-shrink-0 flex md:flex-col justify-end md:justify-center items-end gap-3 mt-4 md:mt-0">
                  {job.link ? (
                    <a href={job.link} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-semibold hover:opacity-90 hover:scale-105 transition-all shadow-md">
                      Apply <ExternalLink className="w-4 h-4" />
                    </a>
                  ) : (
                    <button onClick={() => handleContactPoster(job)} className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold hover:opacity-90 hover:-translate-y-0.5 transition-all shadow-lg shadow-blue-500/25">
                      Contact Poster
                    </button>
                  )}
                </div>
